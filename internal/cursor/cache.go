...
func (p *Provider) cachePath(id string) (string, string) {
	key := fmt.Sprintf("%x", sha256.Sum256([]byte(id)))
	if p.opts.Home == "" {
		return "", key
	}
	return filepath.Join(p.opts.Home, ".cache", "husage", "cursor", key+".json"), key
}

func (p *Provider) restoreUsage(a *subscription.Account) {
	path, key := p.cachePath(a.ID)
	if path == "" {
		return
	}
	f, err := os.Open(path)
	...
	for _, w := range cached.Windows {
		if w.Label == "" || math.IsNaN(w.Used) || math.IsInf(w.Used, 0) || w.Used < 0 {
			return
		}
	}
...
func (p *Provider) saveUsage(a subscription.Account) error {
	path, key := p.cachePath(a.ID)
	if path == "" {
		return nil
	}
