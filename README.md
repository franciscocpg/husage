...
brew install --cask franciscocpg/husage/husage
```
...
The direct provider requests usage at most once every five minutes per configuration during a run and honors longer `Retry-After` delays. Pressing `r` does not bypass this cooldown. Successful Claude usage reads are saved under `~/.cache/husage/claude/` in private files keyed by account and organization. The cache contains usage windows and their original timestamp, never credentials. Failed requests (including rate limits) display the last successful usage with a compact status such as **Claude API · stale data · updated Sep 20, 3:39am**, including after restarting husage. Cached cards omit the detailed error and login command to keep the dashboard compact. When a fresh login is required, a short warning appears above the status: **Login required. Press l to sign in again.** Temporary request failures do not ask you to log in again. A successful refresh replaces the cache and clears the warning. With no successful read yet, the card shows the error without usage. Missing or corrupt caches are ignored; cache write failures leave live usage visible with a warning. Data older than ten minutes is marked stale; a past reset is labeled rather than inventing fresh usage. Missing limits and reset times remain unavailable.
...
Claude refresh tokens are single-use: each renewal replaces the saved refresh token. To keep renewals away from moments when the reply could be lost:

- **Renew ahead of expiry.** husage renews an access token during the last hour before it expires. If this renewal fails, husage keeps using the current token until it expires.
- **Wait after sleep.** When husage detects that the computer has just woken (its wall clock jumped further than its own timer), it waits two minutes before renewing. This avoids starting a renewal during a brief background wake that could end mid-request. The card shows **Login renewal paused after sleep; retrying shortly.**
- **Stop after a rejection.** If Claude rejects the refresh token (HTTP 400/401 or `invalid_grant`), the profile is marked **Login required** and husage stops retrying that token. Repeated retries only add failures, and the Claude CLI can clear the saved login after one. Signing in again saves a new refresh token, and renewal resumes automatically. macOS may ask for access to the existing Keychain item. Credential storage follows [Claude Code's authentication documentation](https://code.claude.com/docs/en/authentication#credential-management).

A missing access token can still be renewed when the saved refresh token and scopes are available. If both tokens are missing, the saved login is reported as incomplete and requires a fresh login. Credential lookup distinguishes a missing login from Keychain access errors, unreadable credential files, and invalid credential data; access errors should be resolved before attempting another login. If a refresh token has expired or been revoked, or its saved scopes are missing, automatic renewal cannot recover it. Press **l** to sign in again from the dashboard, or use `--json` to see the detailed error and profile-specific command. If you log in outside husage, press **r** afterward; husage detects the new credentials without a restart. Profile creation still asks you to confirm the interactive login command before running it. Concurrent husage renewals for the same credential store are serialized with a short-lived lock under `~/.config/husage/locks/`.

To find out why a renewal keeps failing, launch with `--debug`. husage then appends diagnostics to `~/.config/husage/debug.log` (mode `0600`). Each entry covers a renewal attempt for one profile: why renewal started, whether the access and refresh tokens were present, their expiry and scopes, and the Claude CLI version, exit status and output. After a successful renewal, the entry also records whether Claude saved new tokens. Token values never appear. The profile's known tokens, bearer and token-like fields, and long opaque strings are replaced with `[REDACTED]` before anything is written. Native output is still error text from another program, so review the log before sharing it. Without `--debug`, Claude CLI output is discarded.

## Log in again from the TUI

Press **l**, select a subscription with **↑ / ↓**, and press **Enter** to review its native login command. Profiles that require login are selected first. Press **Enter again** to run the command; **Esc** returns to the dashboard without running it.

Claude and Codex reuse the selected profile's existing configuration, including Claude's separate secure-storage override when present. Cursor signs in to its shared native CLI account. This flow does not add entries to `profiles.json`, create another profile, or delete credentials. The native CLI manages its login files.

Background refreshes pause while the login screen is open, and login waits for any refresh already in progress. On success, husage returns to the dashboard and refreshes that profile immediately, bypassing only its cooldown. If login fails or is cancelled, cached usage remains available; press **Enter** to retry or **Esc** to return. The action is unavailable in demo mode and for providers without a supported login command.
...
./bin/husage --debug                        # Log redacted Claude renewal diagnostics
```
...
The workflow uses GitHub's built-in `GITHUB_TOKEN` with release-write permission; no additional repository secret is needed. Stable releases also update `Casks/husage.rb` on `main` in this repository. The explicit tap URL above allows the source repository to serve as the Homebrew tap, so no separate repository or cross-repository token is required. Prereleases leave the stable cask unchanged. The cask is generated by GoReleaser; do not edit its version or checksums for future releases manually. Native provider CLIs are not bundled in the archives.
